  ...
  angular.module('frontend.consumers')
    .controller('ManageBasicAuthXController', [
      ...
      function controller($scope, $rootScope, $log, ConsumerService, MessageService, $uibModalInstance, KongErrorService, _consumer, _cred) {
        ...
        function create() {
          ConsumerService
            .addCredential($scope.consumer.id, 'basic-auth-x', $scope.credentials).then(function (resp) {
            $log.debug("Credentials generated", resp)
            $rootScope.$broadcast('consumer.basic-auth-x.created')
            $uibModalInstance.dismiss()
          }).catch(function (err) {
            $log.error(err)
        ...
        function update() {
          ConsumerService
            .updateCredential($scope.consumer.id, 'basic-auth-x', $scope.credentials.id, $scope.credentials).then(function (resp) {
            $log.debug("Credentials generated", resp)
            $rootScope.$broadcast('consumer.basic-auth-x.created')
            $uibModalInstance.dismiss()
          }).catch(function (err) {
